...
if (sessionStorage.getItem('newCitizen')) {
    ...
    for (let i = 0;i < citizens.length / 2;i++) {
        ...
        let citizenNums = document.createElement('p');
        let tick = '<i class="far fa-check-circle"></i>';
        citizenDiv.classList.add('payee');

        let j = i * 2;
        citizenName.innerHTML = citizens[j];
        citizenNums.innerHTML = citizens[j + 1];

        citizenDiv.appendChild(citizenName);
        citizenDiv.appendChild(citizenNums);
        citizenDiv.innerHTML += tick;

        payeeContainer.appendChild(citizenDiv, document.querySelector('.payee'));
    }
    addTick();
}
...
addButton.addEventListener('click', (e) => {
    if (e.currentTarget.classList.contains('valid')) {
        ...
        // Add payee to session storage
        let oldCitizen = [];
            if (sessionStorage.getItem('newCitizen')) {
                oldCitizen = JSON.parse(sessionStorage.getItem('newCitizen'));
            }
            oldCitizen.push(payeeName.innerHTML);
            oldCitizen.push(accountNum.innerHTML);
            sessionStorage.setItem("newCitizen", JSON.stringify(oldCitizen));
    }
});
